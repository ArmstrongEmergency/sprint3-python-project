...
# Calculate Total Sales of NA, EU & JP of any given game
def calculate_total_sales(game):
    sum = game[NA_SALES] + game[EU_SALES] + game[JP_SALES]
    return sum
sum_call = calculate_total_sales(video_game_sales[0])
print(sum_call)

# Returns games whose genre matches given geenre
def filter_by_genre(data, genre = 'Platform'):
    genre_like = []
    for column in data:
        if column[GENRE] == genre:
            genre_like.append(column)
    return genre_like
print(filter_by_genre(video_game_sales, 'Puzzle'))
print()
...
def get_summary(game):
    ...
    for column in game:
        ...
        year = column[YEAR]
        genre = column[GENRE]
        global_sales = column[GLOBAL_SALES]
    #return f"{name} ({year}) - {genre} - ${global_sales}M"
        print(f"{name} ({year}) - {genre} - ${global_sales}M")
    

get_summary(video_game_sales)
#for value in video_game_sales[0].items():
   # print(value)
