...
# Global Sales above 25 million
for columns in video_game_sales:
    if columns[GLOBAL_SALES] > 25:
        print(f"Name: {columns[NAME]}, Sales: {columns[GLOBAL_SALES]}")

# Games released before the year 2000
pre_2000_count = 0
for column in video_game_sales:
    if column[YEAR] < 2000:
        pre_2000_count = pre_2000_count + 1
print(pre_2000_count)

# North America and Japan sales
north_america_total = 0
japan_total = 0
for column in video_game_sales:
    north_america_total = north_america_total + column[NA_SALES]
    japan_total = japan_total + column[JP_SALES]
print(f"North American Sales: {north_america_total} Japan Sales: {japan_total}")
if north_america_total > japan_total:
    print(f"North America")
else:
    print(f"Japan")

# Games puplished by Nintendo
nintendo_games = []
for column in video_game_sales:
    if column[PUBLISHER] == 'Nintendo':
        nintendo_games.append(column[NAME])
