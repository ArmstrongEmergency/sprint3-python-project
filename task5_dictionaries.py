...
for column in video_game_sales:
    genre_var = column[GENRE]
    global_sales_var = column[GLOBAL_SALES]
    if genre_var not in sales_by_genre:
        sales_by_genre[genre_var] = 0
    sales_by_genre[genre_var] += global_sales_var
...
# 
first_ranked = max(games_per_publisher, key=games_per_publisher.get)
top_game = {}
for column in video_game_sales:
    # Check if this game's publisher is the best publisher
    if column[PUBLISHER] == first_ranked:
        top_game = {
            "name": column[NAME],
            "year": column[YEAR],
            "genre": column[GENRE],
